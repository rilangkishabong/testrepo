$(document).ready(function() {
    ...
    $("#empname_upd").change(function(e) {
        e.preventDefault();
        var usr = $("#empname_upd").val();
        $.ajax({
            type: "post",
            url: "GetUserDatas",
            data: { usr: usr },
            success: function(response) {
                //empdepid
                $("#empid_upd").val(response.usrs.emp_id);
                $("#empdesg_upd").val(response.usrs.desg);
                $("#empdep_upd").val(response.dept[0].dept_name);
                $("#empdepid_upd").val(response.dept[0].id);
            }
        });
    });
    $(".btndeput").click(function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                op1a += "<option value=''>Select Employee</option>";
                for (var i = 0; i < response.allusers.length; i++) {
                    op1a += '<option value="' + response.allusers[i].id + '" ' + (response.allusers[i].id == response.depute_staf_upd[0].dep_emp_name_id ? ' selected ' : '') + '>' + response.allusers[i].name + '</option>';
                }
                $("#empname_upd").html(op1a);
    ...
    $("#deputupdateform").on('submit', function(e) {
        e.preventDefault();
        $.ajax({
            type: 'POST',
            url: "UpdateDeputeDatax",
            dataType: "JSON",
